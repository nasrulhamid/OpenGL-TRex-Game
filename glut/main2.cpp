...
const double tSpeedMin = 1, tSpeedMax = 2, tStep=0.1; 
...
void Timer(int iUnused)
{
		...
		if(obsXpos>0) obsXpos-=tSpeed; else{ 
			obsXpos=80;
			if(tSpeed>tSpeedMin && tSpeed<tSpeedMax) tSpeed++;
		}
...
void processNormalKeys(unsigned char key, int x, int y){
	switch(key) {
		...
		case 'r': case 'R':		// r or R
			...
			break;
		case '1': 
			if (tSpeed>tSpeedMin) tSpeed-=0.1;
			break;
		case '2': 
			if(tSpeed<tSpeedMax) tSpeed+=0.1;
			break;
		case 'y': case 'Y':
			if(stCollision) {
				stCollision=false;
...
//COLLISION DETECTION
	if (positionX+3<obsXpos+6 && positionX+11>obsXpos && positionY<7) {
